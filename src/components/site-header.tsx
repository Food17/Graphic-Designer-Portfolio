...
export function SiteHeader() {
  ...
  const linkClass = (to: string) =>
    `text-sm font-medium uppercase tracking-widest transition-colors ${
      pathname.startsWith(to) ? "text-primary" : "text-muted-foreground hover:text-foreground"
    }`;
    ...
    <header className="sticky top-0 z-50 border-b border-border bg-background/80 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 md:px-10">
        <Link to="/" className="flex items-center gap-3" onClick={() => setOpen(false)}>
          <span className="flex h-9 w-9 items-center justify-center bg-primary font-display text-sm font-black text-primary-foreground">
            MA
          </span>
